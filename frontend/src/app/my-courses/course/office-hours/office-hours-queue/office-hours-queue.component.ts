...
export class OfficeHoursQueueComponent implements OnInit, OnDestroy {
  ...
  //   this.myCoursesService.closeTicket(ticket.id, ticket).subscribe({
  //     next: (_) => this.pollQueue(),
  //     error: (err) => this.snackBar.open(err, '', { duration: 2000 })
  //   });
  // }
  
  /** Refreshes the queue data after a ticket is closed instead of closing ticket a second time */
  onTicketClosed(ticket: OfficeHourTicketOverview): void {
    this.pollQueue();
