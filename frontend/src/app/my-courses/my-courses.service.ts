...
export class MyCoursesService {
  ...
  closeTicket(
    ticketId: number,
    ticketData: OfficeHourTicketTAResponse
  ): Observable<OfficeHourTicketOverview> {
    console.log('recieved ticket', ticketId);
    return this.http
      .put<OfficeHourTicketOverviewJson>(
  ...
  /**
   * Fetches similar tickets
   * @param ticketId: Id of current open ticket.
   * @returns { Observable<OfficeHourTicketOverview[]>}
   */
  seeSimilarTickets(ticketId: number): Observable<{ similar_tickets: OfficeHourTicketOverview[] }> {
    return this.http.post<{ similar_tickets: OfficeHourTicketOverview[] }>(
      `/api/office-hours/ticket/${ticketId}/similar`, 
      {}
    );
  }
}
